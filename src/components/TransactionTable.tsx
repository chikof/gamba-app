...
export default function TransactionTable({
	...
	const extendedTransactions: ExtendedTransaction[] = transactions.map(
		(transaction, index) => {
			...
			const monthlyIncomeBefore = transactions
				.filter((t) =>
					t.date.startsWith(transaction.date.substring(0, 7))
				)
				.slice(
					0,
	...
	const currentTransactions = extendedTransactions.slice(
		startIndex,
		endIndex
	);
						...
						<TableBody>
							{currentTransactions.map((transaction) => (
								<TableRow key={transaction.id}>
									...
									<TableCell>
										{transaction.totalIncomeBefore.toFixed(
											2
										)}
									</TableCell>
									<TableCell>
										{transaction.monthlyIncomeBefore.toFixed(
											2
										)}
									</TableCell>
								</TableRow>
