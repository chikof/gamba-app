...
export default function TransactionTable({ transactions }: { transactions: Transaction[] }) {
  ...
  const extendedTransactions: ExtendedTransaction[] = transactions.map((transaction, index) => {
    const totalIncomeBefore = transactions.slice(0, index).reduce((sum, t) => sum + t.amount, 0);
    const monthlyIncomeBefore = transactions
      .filter(t => t.date.startsWith(transaction.date.substring(0, 7)))
      .slice(0, transactions.findIndex(t => t.id === transaction.id))
            ...
            <TableBody>
              {currentTransactions.map((transaction) => (
                <TableRow key={transaction.id}>
                  ...
                  <TableCell>{transaction.totalIncomeBefore.toFixed(2)}</TableCell>
                  <TableCell>{transaction.monthlyIncomeBefore.toFixed(2)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
