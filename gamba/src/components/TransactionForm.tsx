		...
		<form ref={formRef} action={action} className="space-y-4">
			...
			<div>
						...
						<Command>
							...
							<CommandList>
								<CommandEmpty className="p-4">
									No bookmaker found. Type to add custom.
								</CommandEmpty>
								<CommandGroup>
									{filteredBookmakers.map((bookmaker) => (
										...
										>
											...
											<Check
												className={cn(
													"ml-auto h-4 w-4",
													value.toLowerCase() === bookmaker.toLowerCase()
														? "opacity-100"
														: "opacity-0",
												)}
											/>
				...
				<input
					...
					name="bookmaker"
					value={
						bookmakers.includes(value) ? value : value ? `Custom: ${value}` : ""
					}
				/>
			</div>
			<div>
				<Label htmlFor="amount">
					Amount (positive for wins, negative for losses)
				</Label>
