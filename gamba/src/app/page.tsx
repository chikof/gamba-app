...
import IncomeCharts, { OverallIncomeCard, MonthlyIncomeCard } from '@/components/IncomeCharts';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const { monthlyIncome, overallIncome } = calculateIncome(initialTransactions);
    setTransactions(initialTransactions);
    setMonthlyIncome(monthlyIncome);
    setOverallIncome(overallIncome);
  }, []);

  const handleNewTransaction = (newData: { transactions: Transaction[], monthlyIncome: { [key: string]: number }, overallIncome: number }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto p-4">
        {/* <h1 className="text-2xl font-bold mb-4">Bookmaker Transaction Tracker</h1> */}
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-1/2">
            ...
            <IncomeCharts monthlyIncome={monthlyIncome} overallIncome={overallIncome} />
          </div>
        </div>
      </main>
    </div>
